...
def _joint_pca_rgb(feature_maps: list[torch.Tensor], *, sample_size: int) -> list[np.ndarray]:
        ...
        grid = np.hstack(
            (
                ...
                overlay_header(
                    ...
                    f"frame={frame_id} source={'seed-frame assignment' if is_seed_frame else 'tracking only'}",
                ),
                overlay_header(
                    global_panel,
                    "Global Instances Projected",
                    f"frame={frame_id} view=current primary labels",
                ),
            )
        )
